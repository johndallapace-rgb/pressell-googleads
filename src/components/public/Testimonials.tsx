  ...
  return (
    <section className="bg-white py-12 rounded-2xl shadow-sm border border-gray-100 my-12 mx-4 md:mx-0">
      ...
      <div className="flex flex-col md:flex-row justify-center gap-6 px-6">
        {items.map((t, i) => (
          <div key={i} className="bg-gray-50 p-6 rounded-xl border border-gray-100 flex flex-col h-full max-w-sm mx-auto">
            <div className="flex items-center mb-4 justify-center">
              <div className="flex text-yellow-500">
                {[...Array(5)].map((_, starIdx) => (
                  <svg 
                    key={starIdx} 
                    className={`w-6 h-6 ${starIdx < t.rating ? 'fill-current' : 'text-gray-200'}`} 
                    viewBox="0 0 20 20"
                  >
