...
export async function POST(request: NextRequest) {
  try {
    ...
    const newProduct: ProductConfig = {
      ...
      platform: 'unknown', // Default, can be refined later
      affiliate_url,
      official_url,
      youtube_review_id: youtubeId || undefined,
      
      // Defaults/Fallbacks
      image_url: `/images/default-${vertical.toLowerCase()}.svg`, // Fallback logic handled in UI usually, but good to set default
      headline: `${name} Review: Key Facts, Benefits, and Who It’s For`,
      ...
      // Extended fields (optional but good for template)
      prosCons: {
